class ReservationList {
    constructor(today) {
        ...
        $("#create-reservation-btn").addEventListener("click", this.reservationBtnHandler);
    }
    ...
    reservationBtnHandler(evt) {
        document.location.href = "/reservation/"+$("#selectDate").value;
    }
}

document.addEventListener("DOMContentLoaded", () => {
    const date = new Date();
    new ReservationList(date.getFullYear()+"-"+ (date.getMonth() + 1).toString().padStart(2, '0')+"-"+date.getDate().toString().padStart(2, '0'));
});